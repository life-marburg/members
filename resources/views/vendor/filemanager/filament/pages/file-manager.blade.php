<x-filament-panels::page>
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                @if(count($selectedItems) > 0)
                                    <span class="text-sm text-gray-500 dark:text-gray-400">
                                        {{ __(':count selected', ['count' => count($selectedItems)]) }}
                                    </span>
                            ...
                            <div class="flex items-center gap-3">
                                @if(count($selectedItems) > 0)
                                    ...
                                    <button
                                        ...
                                        wire:confirm="{{ __('Are you sure you want to delete :count item(s)?', ['count' => count($selectedItems)]) }}"
    ...
    <x-filament::modal id="create-folder-modal" width="md">
        ...
        <x-slot name="description">
            {{ __('Enter a name for your new folder') }}
        </x-slot>
    ...
    <x-filament::modal id="move-item-modal" width="md">
        <x-slot name="heading">
            @if(count($itemsToMove) > 0)
                {{ __('Move :count Item(s)', ['count' => count($itemsToMove)]) }}
            @else
                {{ __('Move to Folder') }}
            @endif
        </x-slot>
        ...
        <x-slot name="description">
            {{ __('Select a destination folder') }}
        </x-slot>
    ...
    <x-filament::modal id="create-subfolder-modal" width="md">
        ...
        <x-slot name="description">
            @if($this->subfolderParent)
                {{ __('Create a new folder inside ":name"', ['name' => $this->subfolderParent->getName()]) }}
            @else
                {{ __('Enter a name for your new subfolder') }}
            @endif
        </x-slot>
    ...
    <x-filament::modal id="rename-item-modal" width="md">
        ...
        <x-slot name="description">
            @if($this->itemToRename)
                {{ __('Rename ":name"', ['name' => $this->itemToRename->getName()]) }}
            @else
                {{ __('Enter a new name for this item') }}
            @endif
        </x-slot>
        ...
        <div class="space-y-4">
            ...
            >
                ...
                <div class="space-y-2" wire:loading wire:target="uploadedFiles">
                    ...
                    <p class="text-xs text-gray-500 dark:text-gray-500">
                        {{ __('Please wait while files are being prepared') }}
                    </p>
                </div>
            </div>
            ...
            @if(count($uploadedFiles) > 0)
                <div class="space-y-2">
                    <p class="text-sm font-medium text-gray-700 dark:text-gray-300">
                        {{ __(':count file(s) ready to upload:', ['count' => count($uploadedFiles)]) }}
                    </p>
        ...
        <x-slot name="footerActions">
            ...
            >
                <span wire:loading.remove wire:target="uploadedFiles, uploadFiles">
                    @if(count($uploadedFiles) > 0)
                        {{ __('Upload :count File(s)', ['count' => count($uploadedFiles)]) }}
                    @else
                        {{ __('Select Files First') }}
                    @endif
                </span>
                <span wire:loading wire:target="uploadedFiles">{{ __('Processing...') }}</span>
